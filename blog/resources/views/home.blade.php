        ...
        <div class="col-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    @endif
                    {{-- @if(\App\Models\Theme::where('user_id', Auth::user()->id)->get()) --}}
                    @if($themes !== NULL && !empty($themes))

                    {{-- @if($approve)
                        {{ $themes = \App\Models\Theme::where('user_id', '!=', Auth::user()->id)->orderByDesc('created_at')->get() }}
                    @else
                        {{ $themes = \App\Models\Theme::where('user_id', Auth::user()->id)->orderByDesc('created_at')->get() }}
                    @endif --}}
                        {{-- @foreach(\App\Models\Theme::where('user_id', Auth::user()->id)->orderByDesc('created_at')->get() as $theme) --}}
                        @foreach($themes as $theme)
                        <a href="/theme/{{ $theme->id }}">
                            <div class="card d-flex flex-row justify-content-between align-items-center p-2 mb-3">
